...
- BEFORE calling `run_swarm`, do a QUICK grounding pass — HARD CAP of 3 tool calls (typically one `get_realtime_quotes`/`get_market_data` batch plus at most one `web_search`), then launch `run_swarm` immediately and fold those essentials into the prompt. Workers do their own deep research (news, filings, correlations, analyst views) — do NOT front-load exhaustive due-diligence yourself: burning the iteration budget on collection before the swarm launches is the #1 way these requests fail.
