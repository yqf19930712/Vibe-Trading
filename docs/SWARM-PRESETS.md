...
- **laicai 侧意图声明**（`app/src/server/chat-tools.ts`）：chat 模型通过 `ask_vibe_trading` 的 **`depth="deep_team"` 参数**声明团队研判意图（可选 `swarmPreset` 点名团队）。适用场景两类：① 用户明确要求多视角/团队/委员会式研判；② 重大资金决策（具体金额的加仓/清仓/大类配置）且用户要求全面论证。laicai 服务端据此做两件事：把 `intent=deep_team` + `swarmPreset` 作为结构化字段下发给 router，**并**把固定措辞的 swarm 指令（含点名的 preset）追加进 `query`。
- **结构化字段的作用域止于 router**：`intent` 只决定 router 的预算档（`BUDGET_BY_INTENT`：`standard`=900s / `deep_team`=7200s，租户档 `SWARM_TIMEOUT` env 由同一常量派生；显式 `timeoutS` 仍最优先）；`swarmPreset` 只做形状校验后原样转发。router 把两者与 `deadline_s` 一起放进引擎的 `POST /sessions/<sid>/messages`，但引擎的 `SendMessageRequest` 不声明这两个字段、pydantic 直接忽略——**引擎不消费它们**。引擎侧「开不开 swarm、用哪个 preset」完全由 `query` 里的散文指令 + 系统提示的 Swarm 路由决定，点名 preset 靠 query 里写出 preset 名。
- **散文正则只是兜底**：laicai 保留 swarm 意图正则用于历史线程与用户自己写的散文，声明与嗅探不一致时打一条 `swarm_intent_diverged` 运营事件。
- **两层等待的嵌套关系**（决定上一条是否真的可达）：工具自己的等待是 `cap_timeout(SWARM_TIMEOUT, reserve 90s, floor 60s)`；循环侧的写工具看门狗按 `run_swarm` 声明的 `timeout_seconds`（=`SWARM_TIMEOUT + 120s`）计算并自留 60s。**swarm 自留得更多，所以工具必然先于看门狗自收口**——这是 `wait_budget_exhausted` 打捞路径可达的前提，回归测试 `agent/tests/test_swarm_timeout_nesting.py`。
- 等待预算耗尽时 run **不取消、后台继续**（`wait_budget_exhausted`），工具返回 run_id 与部分结果；模型可据此如实汇报，或用 `run_swarm(run_id=…)` 继续等**同一个** run（不起新 run、不产生额外 worker token）。attempt 被取消（router 未答即 cancel、用户删除对话）时等待循环在 ≤1s 内醒来返回 `cancelled_wait`（同样带 run_id、run 不动），不再空等到两小时预算耗尽。
